...
\todo[inline]{Doplnit PathType o boolean for bound, volania zoznam funkcii volanych na tuto path.}
...
\IF{$is(node, FLWORExprNode)$}
    ...
    \FORALL{$bindingNode \in node.getChild(TupleStreamNode).getChildren()$}
        \IF{$is(bindingNode, ForClauseNode)$}
            ...
            \IF{$getType(expr) == PathExpr$ without predicates except the last step, using only child and descendant axes}
