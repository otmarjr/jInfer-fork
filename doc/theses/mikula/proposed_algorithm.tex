...
\item $c: V \rightarrow C$ is function assigning each node with its class from set $C$ of all XQuery language constructs listed in Figures \ref{FIG_syntax_tree_node_types_part_1}, \ref{FIG_syntax_tree_node_types_part_2}, \todo{Tohle je hrozné peklo. Potřebujeme to takto? Nedalo by se to nacpat do 1-2 tabulek/obrázků? Je nutné aby každá položky byla na samostatném řádku. Trochu to vypadá jako umělé natahování diplomky.}
...
\IF{$is(node, FLWORExprNode)$}
    ...
    \FORALL{$bindingNode \in node.getChild(TupleStreamNode).getChildren()$}
        \IF{$is(bindingNode, ForClauseNode)$}
            ...
            \IF{$getType(expr) == PathExpr$ without predicates, using only child and descendant axes}
...
\IF{$is(exprNode, OperatorNode$)}
    \IF{$exprNode.operator == GEN\_EQUALS$}
        \FORALL{$var \in forVars$}
            \IF{$exprNode$ is of a form $var/L = C$ where $L$ is a path without predicates using only child and descendant axes, and getType(C) is a constant}
...
\todo[inline]{Doplnit algoritmus comparison with a constant pre predikaty.}