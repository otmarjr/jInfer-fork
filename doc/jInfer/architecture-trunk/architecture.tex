...
For these cases, special \code{getMutable()} method is implemented to obtain regexp with none of the members set. One has to fill in all properties carefully and call \code{setImmutable()} aftewards. Proper usage should be one of following:
\begin{verbatim}
...
%\end{figure}
\begin{figure}
\centering\includegraphics[width=\textwidth]{nodes_full2}
...
The same \code{get\.Mu\.tab\.le()} principles and good usage practises hold for these classes.
...
\end{figure}
The process by which jInfer infers the resulting schema from various inputs (inference process) is summarized by fig. \ref{inference_process}. From the high-level viewpoint, it consists of four consecutive steps carried out by four different modules:
\begin{enumerate}
	\item Initial Grammar (IG) generation: done by the \jmodule{Initial Grammar Generator (IGG)} module, this is the process of converting of the inputs to IG representation. Documents, schemas and queries selected as input are evaluated, simple rules are extracted and in the end sent to the next step. Not all types of inputs have to be used to create the initial grammar, as will be described in step 3.\\
...
<person>
  ...
  \end{verbatim}
	...
	\item Non-grammatical input processing: done by the \jmodule{Non-grammatical Input Processor}, in this step, certain inputs are exploited to extract any non-grammatical information that can help in the inference. The information is then merged into the grammar which is sent to the last step. Inputs processed in this step can be inputs that were used to create the initial grammar, and, as well inputs that were not touched by the \jmodule{IGG} module. This step is not a crucial part of the inference, it's purpose is to provide a possibility to easily extend the inference process by incorporating other non-grammatical inputs. Therefore, the inputs and the way of merging the extracted information with the grammar are not specified in detail and they depend on an implementation.
...
\begin{itemize}
	...
	\item \code{NonGrammaticalInputProcessor}
...
\jmodule{Non-grammatical Input Processor} takes both grammar and \code{Input} object.
...
\begin{itemize}
	...
	<!ELEMENT more EMPTY>
	\end{verbatim}
	\emph{Elements} note and more are empty, but we cannot use lambda regexp. Import will use empty concatenation instead.
	For \emph{element} person, import module has to generate some positive examples of language generated by regular expression \verb+(info?, more, more?, more?)+.
	
This is basically done by calling \code{cz.cuni.mff.ksi.jinfer.base.interfaces.Expander} interface method \code{ex\.pand()}. One can implement own intelligent expander, or use our provided \code{cz.cuni.mff.ksi.jinfer.basic\.igg.ex\.pansion.ExpanderImpl}.
...
\code{IGGeneratorCallback.finished()} and \code{SimplifierCallback.finished()} have a grammar (Initial Grammar in case of IGG) as their only argument. \code{NonGrammaticalInputProcessorCallback.finished()} has that argument, as well.
...
\begin{enumerate}
	...
	\item This again causes \code{Runner} to encapsulate and execute NonGrammaticalInputProcessor's \code{start}, passing the simplified from the second callback and the third callback as parameters.
	\item When NonGrammaticalInputProcessor finishes, it invokes callback's \code{finish} method, passing the merged simplified grammar as parameter.
	\item This once again causes \code{Runner} to encapsulate and execute SchemaGen's \code{start}, passing the grammar from the third callback and the fourth callback as parameters.
...
if (!RunningProject.getNextModuleCaps().getCapabilities()
  ...
  // lookup expander
  Expander expander = Lookup.getDefault().lookup(Expander.class);
  // return expanded
...
\begin{itemize}
	\item \jmodule{Runner}: as described in the section \ref{section_runner}, this module encapsulates the inference process (including the fact that inference consists of 4 phases as described before). Most important class: \code{Runner}.
	...
	\item \jmodule{XQueryImporter}: XQuery input handling incorporating lexical and syntax analyses as proposed in \cite{mikula}.
	...
	\item \jmodule{BasicXQueryProcessor}: a \jmodule{Non-grammatical Input Processor} implementation. Implements algorithm proposed in \cite{mikula} processing XQuery queries as an non-grammatical input and refining the inference process by information extracted from the queries.	
...
\end{itemize}
The dependencies between all the modules are summarized in figure \ref{module_deps}. Figure \ref{modules_inference} shows modules implementing the 4 main inference interfaces described in \ref{section_inference_process}.
...
jInfer uses tokens in the following way: \jmodule{Runner}, as the module encapsulating the inference process, and \jmodule{ProjectType} as an encapsulation of jInfer projects both require the following 4 tokens.
\begin{itemize}
	...
	\item \code{cz.cuni.mff.ksi.jinfer.base.interfaces.inference.NonGrammaticalInputProcessor}
...
\paragraph{}
Another use of tokens is \code{cz.cuni.mff.ksi.jinfer.base.interfaces.RuleDisplayer} representing a module providing a rule displayer. This token is provided by \jmodule{BasicRuleDisplayer} and \jmodule{TreeRuleDisplayer}, requested by \jmodule{BasicIGG} and \jmodule{TwoStepSimplifier}.
