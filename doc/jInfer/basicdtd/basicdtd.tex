...
Elements are processed in method \code{elementToString()}, but the real work is done in method \code{regexpToString()}, which takes regexp and recursively converts it to string representation.
But before, whole element is sent to method \code{expandIntervalsElement} of class \code{IntervalExpander}.
...
First, element is passed to \code{expand\.In\.ter\.vals\.E\.le\.me\.nt}, which does (shortened):
\begin{verbatim}
...
\end{verbatim}
It calls private method \code{expandIntervalsRegexp} to handle regexp in that element.
This private method makes big switch according to regexp type.
...
If it is unbounded, attach the regexp once, with interval $\{0, \infty\}$.

After intervals are expanded, further processing in \code{regexpToString} is divided by big \code{switch} statement of type of regexp.
