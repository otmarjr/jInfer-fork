...
% TODO vito Sorry, I don't understand the following 3 sentences, so I cannot correct them
...
% TODO vito Don't understand the following two sentences, please rephrase
After clustering, implementation should keep the clusters in member, as it will be later requested by calling method \code{T getRepresentantForItem(T item)}.
...
We cluster elements in \code{Iname<AbstractStructuralNode, Attribute>} class (Iname je clusterer!), which takes \code{AbstractStructuralNode} classes to cluster as main, and \code{Attribute} classes as attributes. % TODO anti I don't understand the sentence, rewrite.
...
To automaton, instead of instances on right side of rules, their cluster representants are inserted as one can see in section \ref{section_AutomatonMergingState_structure}. % TODO vito Don't understand the sentence
% Do automatu sypeme concat, ale nie jeho prvky, ale ich reprezentantov
