...
\section{Motivation}
There is a large space of possible XQuery utilizations, but obviously, they all cannot be covered by one thesis. Hence, we had to choose only some of them. We decided to focus on an inference of XSD built-in atomic types of elements and attributes, and a key discovery.
...
\todo[inline]{Ako je to s osou descendant or self pri hladani join patternov? Orig. metoda o nej explicitne nehovori, ale zrejme pouziva}
\todo[inline]{Ako je to s attribute osou v L cestach join patternov? Atribut predsa moze byt kluc.}
...
\todo[inline]{Definicia NUS a context Path, podobne ako v klucoch.}
