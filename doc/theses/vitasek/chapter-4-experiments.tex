...
\begin{figure}
  ...
  \centering
    \subfigure[\jmodule{OVA1}]{\includegraphics[width=0.45\textwidth]{images/experiments/data/realistic/OVA1}}
    \subfigure[\jmodule{OVA2}]{\includegraphics[width=0.45\textwidth]{images/experiments/data/realistic/OVA2}}
    \subfigure[\jmodule{OVA3}]{\includegraphics[width=0.45\textwidth]{images/experiments/data/realistic/OVA3}}
    \subfigure[\jmodule{XMA-c}]{\includegraphics[width=0.08\textwidth]{images/experiments/data/realistic/XMA-c}}
    \subfigure[\jmodule{XMA-p}]{\includegraphics[width=0.08\textwidth]{images/experiments/data/realistic/XMA-p}}
    \subfigure[\jmodule{XMD}]{\includegraphics[width=0.45\textwidth]{images/experiments/data/realistic/XMD}}
\end{figure}
...
\begin{figure}
  ...
  \centering
    \subfigure[\jmodule{MSH}]{\includegraphics[width=0.08\textwidth]{images/experiments/data/converted/MSH}}
    \subfigure[\jmodule{NTH}]{\includegraphics[width=0.45\textwidth]{images/experiments/data/converted/NTH}}
\end{figure}
...
Also, we will need data of comparably similar characteristics but varying size, to study the effects of size on the run times of experiments. For this reason we created 11 more sets, from \jmodule{0-0} as the trivial one to \jmodule{100-500} as the biggest one. From now on, these will be referred to as \textit{sized test data (sets)}. We wanted to keep density the same among these sets, and we picked the $\frac{|E|}{E_{max}}$ density interpretation for this.
...
\begin{figure}
  ...
  \centering
  	\subfigure[\jmodule{48-80}]{\includegraphics[width=\textwidth]{images/experiments/data/artificial/size/40-80}}
	\subfigure[\jmodule{70-245}]{\includegraphics[width=\textwidth]{images/experiments/data/artificial/size/70-245-fixed}}
\end{figure}
...
Our investigation into various CH will start by comparing \jmodule{FIDAX} from the original article \cite{fidax} to 2 of our trivial randomized hungry heuristics, \jmodule{Random} and \jmodule{Fuzzy}.
...
There is a minor question to be answered easily: is \jmodule{FIDAX} ``hungry enough"? Could we improve it using \jmodule{Hungry} as IH? Let us design a short experiment.
...
We won't be needing pool bigger than one, nor more iterations - both \jmodule{FIDAX} and \jmodule{Hungry} are deterministic. We will try all official data sets, first with empty IH, second with \jmodule{Hungry} as IH. We will gather the qualities in each case and see whether there is any improvement.

The experimental results are summarized in the Table \ref{table-experiments-fidax-and-hungry} and are quite surprising. As trivial a heuristic \jmodule{Hungry} is, it is still able to improve the ID set found by \jmodule{FIDAX} by as much as almost 50\% (the last row, \jmodule{100-1000}).

Table \ref{table-experiments-fidax-and-hungry-idsets} lists the ID attributes found in both cases for this most extreme input, \jmodule{100-1000}. Note that the content of each cell means ``attribute \texttt{attr} in element \texttt{vertexXY} should be marked as ID attribute".

\begin{table}
  \caption{Results of adding \jmodule{Hungry} after \jmodule{FIDAX}}
  \bigskip
...
\begin{table}
  \caption{ID sets in \jmodule{FIDAX} versus \jmodule{FIDAX} + \jmodule{Hungry}}
  \bigskip
