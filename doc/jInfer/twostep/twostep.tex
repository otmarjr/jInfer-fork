...
Then, empty list of elements is created as the simplified grammar.
...
type \code{T} (that is representative of some main cluster) has some "attribute" clusters associated with it.
...
Clusterer can tell you the representative for any element in the whole input (not only those elements in rules, but also those that may be on right side of rules). Also, clusterer (if it is with attributes) has information about attributes in each cluster.
