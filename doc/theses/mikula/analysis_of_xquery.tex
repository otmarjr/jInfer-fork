svn diff\chapter{Analysis of XQuery} \label{CHAPTER_analysis_of_xquery}
...
A common three-way join. \todo{Tady asi něco chybí.}
...
Usage of \texttt{for} construct indicates that a sequence of \texttt{book} elements which satisfy the condition in \emph{where} \todo{tady se nám nějak změnil styl} clause is expected. It is a condition composed of two single conditions joined by \emph{and} logical operator. Therefore, in order to satisfy the whole condition, both single conditions must be satisfied as well. Thus, it is expected that several elements satisfy each of the single conditions.
...
\todo[inline]{Tady to chce ějaký komentář.}
