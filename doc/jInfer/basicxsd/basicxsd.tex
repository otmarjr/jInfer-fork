...
Before we describe the export of elements, let's take a look on how we define elements and their attributes using XSD language. This subsection covers only XSD features that the exporter supports. For list of supported features, see TODO ref.\\
...
\begin{verbatim}
public AbstractElementsExporter(final PreprocessingResult preprocessingResult,
                                final Indentator indentator)
\end{verbatim}

Names of parameters are self explanatory. Parameter \code{preprocessingResult} is a result of preprocessing. Parameter \code{indentator} is an instance of \code{Indentator} class to be used to buffer and indent output of exporter. This instance has not to be empty. Output of exporter is appended at the end of text held by the \code{indentator}. This behaviour is convenient when chaining output of several elements exporters.\\
...
If a global type contains another elements, these are processed as following.

\begin{itemize}
	\item If a contained element is global or of a built-in type, its type is simply referenced.
	\item If a contained element is not global nor of a build-in type, it is fully defined in a place of its occurrence inside the global type.
\end{itemize}
...
Examples of definition of global types.
...
This module is just basic implementation so many XSD features are not supported. List of supported XSD elements and their attributes follows. Let \code{xs} be XMLSchema namespace.
...
This problem appears if input grammar contains an alternation of two or more concatenations, which begin with at least one same element. Part of a problematic XSD is for example following.
