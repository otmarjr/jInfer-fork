...
The first step of the algorithm involves lexical and syntactic analyses known from compilers and produces a so-called syntax tree. The analyses are taken from Ji\v r\'{i} Schejbal's master thesis \todo{ref}. Since they are not directly related to the inference, and thus, they are not directly related to the topic of this thesis, we will not describe them in this thesis. Nevertheless, they provides us with a helpful processing of XQuery queries and we can focus on the inference.
...
\todo[inline]{Co su to za typy?}
\todo[inline]{Preco vlastne tuto analyzu robime?}
...
A similar approaches as in the case of functions can be applied to determine types of global variables. Alike the functions, the global variables are defined in the prolog section. A type of a variable can be explicitly specified in the definition \todo{je to tak? priklad}. If it is not it may be often deducible from the binding expression. Again, we do not analyse external variables for the same reason of simplicity as the external functions.
...
In this step, the algorithm transitions through the syntax tree and tries to infer types of elements and attributes from expressions. This step could be merged with the step 2 where the syntax tree is being transitioned but for better comprehension we present it separately.

How are the types inferred from the expressions? Only expressions of a particular type are exploited. Specifically, the expression has to contain a subexpression representing an element or an attribute or a set of elements or attributes. In addition, we have to known what are the elements or the attributes, what their names are, how their were retrieved. Basic example of such expression is a path expression. Besides, the expression has to be either a function call or an arithmetic operation \todo{presnejsie} \todo{Nejake ine?}.

\todo[inline]{Popisat odvodzovanie z volania funkcie}
\todo[inline]{Popisat odvodzovanie z aritmetickej operacie}