...
Specifically, it is desired to determine the XML Schema built-in types \todo{link do appendixu} of expressions, if it is possible. Also, for each path expression we want to determine if it selects elements or attributes and we want to store the entire path in the type information. This information can be used for example in a $FLWOR$ expression \todo{priklad nejakeho FLWOR a ukazat ako vieme zistit, ktore elementy su prechadzane}. \todo[inline]{Uistit sa, ze takto nam to staci.}

To capture sequences, we assign each type with its cardinality as proposed in \todo{ref na Schajbalovu diplomku}. The cardinality expresses one of the following five sequence types.

\begin{itemize}
\item An empty sequence.
\item A sequence of exactly one item.
\item A sequence containing zero or one item.
\item A sequence containing zero or more items.
\item A sequence containing one or more items.
\end{itemize}
