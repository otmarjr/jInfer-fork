...
If the operator is one of $+$, $-$, $*$, $div$, $mod$ (the class of the expression node is $Operator$ and it represents one of $PLUS$, $MINUS$, $MUL$, $DIV$, $MOD$ (for all operators, see Attachment \ref{FIG_operators})), one operand is $E$ and the type $T$ of the other operand is one of numeric built-in types, then $T$ is also the inferred type of $S$.
...
If a currently processed expression is a function call of an user-defined function and at least one of its real arguments is an instance of \texttt{PathType}, get the function declaration node. In the function body, replace the formal arguments by the real arguments and set them as the local variables, so the proposed algorithms can be applied to process the function's body. Analyse the types of expressions in the body with the new information on the types of arguments by incorporating of the proposed algorithms and replace the function's return type by the one currently determined.
...
\IF{$lastStepNode.hasPredicates()$ $==$ false}
	\RETURN false
\ENDIF
\IF{number of item in list $lastStepNode.getChild(PredicateListNode)$ $.getChildren()$ is higher than 1}
	\RETURN false
\ENDIF
...
\todo[inline]{Upravit styl vsetkych algoritmov (null, ==, !=, true/false, ...) tak ako je tu.}
...
\IF{$is(node, ExprNode)$}
	...
	\IF{$type$ is $PathType$ AND $isTargetPath(type, varName)$}
...
To define a key and a foreign key, we adopt the definitions introduced in the original approach \cite{Necasky:2009:DXK:1529282.1529414} with a modification that XPath paths are replaced by \texttt{PathType} instances.

\begin{define}[Key]
A key is a construct $(C, P, \{L\})$ where $C$, $P$ and $L$ are \texttt{PathType} instances without predicates and without special function calls that use only child and descendant axes. $C$ is called \emph{context path}, $P$ \emph{target path} and $L$ \emph{key path}. $C$ can be omitted, i.e. we can write $(P, \{L\})$. This is equivalent to $(/, P, \{L\})$. If $C$ is omitted we call the key \emph{global key}. Otherwise, it is called \emph{relative key}.
\end{define}

\begin{define}[Foreign key]
A foreign key is a construct $(C, (P_1, \{L_1\}) \rightarrow (P_2, \{L_2\}))$ where $(C, P_2, \{L_2\})$ is a key and $P_1$ and $L_1$ are \texttt{PathType} instances without predicates and without special function calls that use only child and descendant axes. $C$ can be omitted as in the case of keys.
\end{define}
...
\FORALL{$bindingNode \in bindingNodes$}
    ...
    \IF{$type$ is $PathType$ $AND$ $usesOnlyChildAndDescendantAxes(type)$ $AND$ $isWithoutPredicatesExceptLastStep(type)$}
    	...
    	\IF{$P$}
        	...
        	\ENDFOR
        \ELSIF{$isWithoutPredicates(type)$}
...
\IF{$is(node, FLWORExprNode)$}
    ...
    \FORALL{$bindingNode \in node.getChild(TupleStreamNode).getChildren()$}
        \IF{$is(bindingNode, ForClauseNode)$}
            ...
            \IF{$getType(expr)$ is $PathType$ AND $usesOnlyChildAndDescendantAxes(getType(expr))$}
...
\IF{$is(exprNode, OperatorNode$)}
    \IF{$exprNode.operator == GEN\_EQUALS$}
        \FORALL{$var \in forVars$}
            \IF{$exprNode$ is of a form $var/L = C$ where $getType(var/L)$ is $PathType$ AND $isWithoutPredicatesExceptLastStep(getType(var/L))$ AND $usesOnlyChildAndDescendantAxes(getType(var/L))$ AND $C$ is a literal constant}
