...
\ELSIF{$exprNode$ represents operator $op$}
     \IF{$op$ is a comparison operator}
         \STATE $type = xs:boolean$
     \ELSIF{$op$ is an additive operator $AND$ one of the operands is of a numeric type}
         \IF{both operands are of numeric types}
...
\todo[inline]{algoritmus 6.2, riadok 2, slovo "represents" - Tady by neškodilo definovat nějakou funkci a její možné návratové hodnoty a toto používat.}
\todo[inline]{algoritmus 6.2, riadok 19 - Totéž, neškodila by nějaká funkce, tj. trochu formálnější zápis. Jednak se pravděpodobně bude hodit dál a druhak to zpřehlední popis.}
