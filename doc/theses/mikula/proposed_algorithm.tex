...
\todo[inline]{Niektore typy mozu byt take, ze pri prvom prechode ich nevieme zistit, ale ked uz mame zistene ine typy, tak dokazeme zistit aj tie, co sme pred tym nevedeli. Takze by bolo treba robit viac priechodov, ale skor nieco ako dopologicke triedenie, co zavisi na com, a potom typy zistovat v tomto poradi (mozno aj v druhom kole).}
...
\begin{itemize}
\item $pathExprNode$ - An instance of $PathExprNode$.
\item $substeps$ - An association between references from $stepNodes$ and instances of $PathType$ type.
...
As described in the original method and summarized in Chapter \ref{Keys} we recognize two cases (O1) and (O2) of inferrence of keys from occurrences of the join patterns, and rules (R1 - R5) to classify each occurrence into one of these cases. For the join pattern 3, we introduce a new rule, considering the join pattern 3 of case (O1), assigned with weight of 0.5. \todo{Popisat dovod nizsej vahy (nie je take jasne, ze je to par key-foreign key), pripadne zmenit vahu.}
...
\todo[inline]{Popisat? To by som musel asi jedine skopirovat z clanku, to vlastnymi slovami nepopisem.}
...
The first XQuery construct utilized by this step is a function call of $distinct-values$ function as discussed in Section \ref{Negative_uniqueness} and shown in Algorithm \ref{ALG_rejection_of_uniqueness_aggregation_functions}. The algorithm searches for calls of the mentioned functions with a $PathType$ on their input. Elements and attributes selected by those $PathTypes$ are then considered to be not unique, and consequently, they cannot be keys.
...
\todo[inline]{Znovu pouzitie originalnej metody. Popisat?}
