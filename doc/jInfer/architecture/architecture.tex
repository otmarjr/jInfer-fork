...
An inference module can query the capabilities of the \textit{next} module in the inference chain by invoking the \code{RunningProject.getNextModuleCaps()} method to obtain the next module encapsulated in the \code{Capabilities} interface.\\
...
% TODO vektor check later - didn't this change?!?
...
jInfer, being a NetBeans Platform application, is divided into multiple \textit{NetBeans Modules}. It is very important to understand that this division is in principle orthogonal to that naturally sketched in the previous chapters - theoretically, the whole jInfer could be contained in a single NB module, because the framework uses Lookups to locate logical modules. However, for the sake of organization, jInfer is split into several modules and most of the time there is a strong correlation between logical units of jInfer and its NB modules.\\

A short overview of these modules and their functions follow. To get a deeper understanding of any of them, please refer to its documentation.\\
...
\begin{itemize}
	\item \jmodule{Runner}: as described in the section TODO anti link, this module encapsulates the inference process (including the fact that inference consists of 3 phases as described before). Most important class: \code{Runner}.
	...
	\item \jmodule{XSDImportSAX}: XSD input handling using SAX approach.
	\item \jmodule{XSDImportDOM}: XSD input handling using DOM approach.
	\item \jmodule{TwoStepSimplifier}: a \jmodule{Simplifier} implementation. TODO anti
...
\begin{itemize}
	\item \jmodule{RuleDisplayer}: module capable of displaying a grammar in a graphical way.
	\item \jmodule{AdvancedRuleDisplayer}: more advanced version of the former using \jmodule{JUNG}'s graphing capabilities.
	\item \jmodule{AutoEditor}: encapsulation of an interactive finite state automaton editor.
	\item \jmodule{JUNG}: wrapper module for the JUNG TODO anti link graphing library.
...
% TODO module dependency hierarchy
% TODO picture showing out inference modules in a humble graph

To get deeper understanding of NBP modules, their versions, dependencies etc, refer to http://bits.netbeans.org/dev/javadoc/org-openide-modules/org/openide/modules/doc-files/api.html . TODO anti Cite
