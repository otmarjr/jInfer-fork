...
\STATE $ES \gets \emptyset$
\FOR{$i = 1 \to 20$}
	\FOR{$file \in $ sized artificial data}
...
\begin{itemize}
  ...
  \item Big fight Fuzzy vs. Random vs. FIDAX take note of quality (mean, variance) but also time (again, mean)
  ...
  \item this can be done inside the GLPK version comparison! GLPK: Input -> How long the search for optimum lasts - this shows we cannot rely on GLPK alone! it just takes too long.
	\item this can be done inside the GLPK version comparison! GLPK: Time limit -> Quality
