...
The pattern occurrence is considered of case (O1) if it is the for join pattern (weight: 1), aggregation function \texttt{avg}, \texttt{min}, \texttt{max} or \texttt{sum} is applied on a return path (weight: 1), aggregation function \texttt{count} is applied on a return path. \todo {definovat return path?}
