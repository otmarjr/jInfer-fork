...
Global elements (retrieves from the result of preprocessing) are defined by referencing its type, which is supposed to be defined globally. TODO example. This applies also to elements which are not global and their type is one of a built-in types. TODO example.

Elements which are not global and are not of a build-in type are defined recursively, at the place of their occurrence. TODO example.

\subsubsection{Export of attributes}

TODO translate. Export elementov zahrna taktiez export ich atributov, kedze atributy sa exportuje ako element <xs:attribute> v elemente <xs:complexType>.

\subsection{Preferences}

All settings provided by \jmodule{BasicXSDExporter} are project-wide, the preferences panel is in \code{cz.cuni.mff.ksi.jinfer.basicxsd.properties} package. It is possible to set the following. 
\begin{itemize}
	\item Turn off generation of global element types.
