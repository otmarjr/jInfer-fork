...
Regular expression is implemented as class \code{Regexp<T>} with supplementing classes \code{RegexpInterval} and \code{Reg\.exp\.Type}.
...
methods \code{get\.To\.ken(), get\.Con\.ca\.te\.na\.tion()} etc.
...
Finally, our interface/class model for representing XML nodes is drafted on fig. \ref{nodes}. Those, who are brave enough, can look on fig. \ref{nodes_full2}.
In result, \code{Element} and \code{SimpleData} have method \code{getType()} to devise type of \code{Abstr\.act\.Stru\.ctu\.ral\.Node} variables. And \code{SimpleData} and \code{Attribute} have methods \code{getContentType()} and \code{getContent()} to work with content model.
