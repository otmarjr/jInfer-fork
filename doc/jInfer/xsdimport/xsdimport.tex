...
None of the modules provide nor require any tokens. Complete dependency tree is depicted in Figure 9 and described in \cite[section 4]{archdoc}.
...
Rule-trees are tree data structures, where vertices are instances of \code{Element} class and edges are the relations between them stored in \code{Regexp} instances within vertices. Every vertex is a rule by itself, so in order to obtain IG rules, one must extract all Elements from a rule-tree. Rule-trees are created by \code{DOMHandler}, described in section \ref{sect:domhandler}. The extraction is done by \code{DOMParser}, described in section \ref{sect::domparser}. Or alternatively, when parsing with SAX is selected, \code{SAXHandler} handles both processes.
...
Functionality is divided between separate packages, in the same order:
...
Simmilar to the \code{Processor} implementations in the \code{BasicIGG} module, this is the core class of \code{XSDImporter} logic.
XSDProcessor registers itself into \emph{schema} input folder, \emph{xsd} file extension and declares it cannot handle other file extensions.\\
...
Initial grammar generated by this processor is considered \emph{complex} and should be followed by expansion (see \cite[section 4]{basiciggdoc}).
...
Package \code{cz.cuni.mff.ksi.jinfer.xsdimporter.utils} contains classes available for all parsing methods.

- TODO explain the purpose of utility classes \\
- TODO describe every helper class in importer?
...
\begin{itemize}
	...
	\item \code{isVerbose()} returns \code{true} or \code{false}, depending on the checkbox.
	\item \code{stopOnError()} returns \code{true} or \code{false}, depending on the checkbox.
\end{itemize}
...
\end{tabular}
\\
This module implements the import functionality using API from Xerces DOM parser. Its structure is very simple, all classes are grouped in one main package, relying on utility classes from \code{XSDImporter} module mentioned before.
...
BuildRuleSubtree is the most important method. It is divided into four logical parts
\begin{enumerate}
...
\begin{enumerate}
	...
	\item If no errors occured during this process, list of extracted rules is returned to \code{IGGeneratorImpl} for expansion. Otherwise, depending on the \emph{Print verbose info} setting, the inference is stops, or an empty list of rules for this input document is returned.
\end{enumerate}
...
\subsection{Data Flow using SAX parser}
