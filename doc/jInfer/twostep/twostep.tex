...
In \jmodule{TwoStepSimplifier} we design service providing factory interfaces so that they extend \code{NamedModule}, \code{Capabilities} and \code{UserModuleDescription}.
...
In this place, it is interestesting to look on method \code{buildPTAOnSymbol}.
...
We have implemented \emph{k,h-context} state equivalence (see \cite{ahonen}) in class \code{KHContext} (implements \code{MergeConditionTester} interface), which is used by \code{Greedy} to test mergability of states by default configuration.
...
This implementation has methods \code{createSuperInitialState} and \code{createSuperInitialState}.
...
\begin{enumerate}
	...
	\item Collapse all {in | out}-transitions, which same {source | destination} state to one transition with \code{Regexp.ALTERNATION} of original regexps.
	\item For each in-transition, loop, and out-transition, add transition from in-transition.getSource to out-transition.getDestination with symbol of \code{Regexp.CONCATENATION} of in-transition regexp, loop regexp and out-transition regexp. Thus by-passing the state completely.
...
\begin{figure}
	...
	\subfigure[Bypass state 3 by adding concatenation transitions from states 1 and 2 to state 4.]
	{
