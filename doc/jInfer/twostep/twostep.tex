...
% TODO anti Representant -> Representative!
...
% TODO anti Sorry, I don't understand the following 3 sentences, so I cannot correct them
...
For now, it is not important how the worker interface looks like, lets just examine the factory.
...
% TODO anti Rewrite so that these sentences don't end with "too" all the time
Under certain circumstances it is useful to declare method \code{create()} generic.
The \jmodule{AutomatonSimplifier} works with \code{Automaton} instances, which are generic too.
But automaton simplification does not depend on type of symbol of automaton transitions, so interface \code{AutomatonSimplifier<T>}
generic too.
Factory interface deals with this by defining the \code{create()} method generic.
...
Lets look at \code{AutomatonMergingStateFactory}. This is factory of module which has \jmodule{AutomatonSimplifier} as a submodule.
...
Thorough this document, we will mention only worker interface when describing submodule, since all factory interfaces are designed the same way as was just described.
...
The latter is registered as service provider (and implements) of \code{Simplifier} interface (defined in package \code{cz.cuni.mff.ksi.jinfer.base.interfaces.inference}).

Its main method is \code{start()} which receives Initial Grammar in form of
\begin{verbatim}
List<Element> grammar
\end{verbatim}
Each grammar rule is represented as class \code{Element}, where element is left side of rule and its \code{getSubnodes()} method
returns regular expression representing right side of rule (as described in \cite[3.1.1]{archdoc}, they are all concatenations).
Second parameter is 
\begin{verbatim}
...
That's all, the magic is in \code{TwoStepSimplifierFactory} class.
...
% TODO anti Rename finalGrammar to resultGrammar or something like that
\begin{verbatim}
// 1. cluster elements
...
Then, empty list of elements is created as the final (simplified) grammar.
...
Finally, regular expression cleaning is done in a submodule and new \code{Element} instance is created as a copy of processed node, but with inferred and cleaned regexp.

\paragraph{Sentinel processing}
There are some specialities in processing sentinel elements (see \cite[3.2]{archdoc}).
Sentinels can be only on right side of rules and they have to be sent to clusterer, as someone further in the chain can ask for cluster of sentinel element - which appears on right side.
But sentinels have no content, they must not be taken into account when inferring attribute properties (whether it is required or optional) - since sentinels have no attributes at all.
...
This class simply holds a \code{Set} of members of this cluster and one of the references as \code{representant} member.

Any clusterer has to comply to \code{Clusterer} interface.
Its purpose is to cluster bunch of elements (rules) on input, into bunch of \code{Cluster} class instances (clusters) on output.
...
% TODO anti Don't understand the following two sentences, please rephrase
...
Missing an item probably indicates error in algorithm rather than normal workflow.
...
% TODO anti Not clear what "worker" is
...
\end{figure}
Process illustrated in fig. \ref{twostep_flow} is as follows.
\begin{enumerate}
	\item Initial Grammar on input, which is ruled into \jmodule{Clusterer}. % TODO anti WTF is "ruled into"?
...
We cluster elements in \code{Iname<AbstractStructuralNode, Attribute>} class, which takes \code{AbstractStructuralNode} classes to cluster as main, and \code{Attribute} classes as attributes. % TODO anti I don't understand the sentence, rewrite.
Clustering is done based on element's \code{getName()} equality (ignoring case).
For each rule (element), it is first clustered by finding the cluster which representant has the same name - or create new cluster with this element as representant.
...
\begin{itemize}
  ...
  \item If it is \textbf{element} and it is tagged as sentinel - search main clusters to find cluster with representant of same \code{getName()}, or create new cluster with this sentinel as representant. From IGG, sentinels may be only on right sides of rules and since each element in schemas has to be defined, there must exist another element with same name, which is not sentinel (and maybe it will come to process in future). So there can't be cluster with only one sentinel element in it. % TODO anti Please rewrite the really long sentence.
  \item Do nothing otherwise, since it is element, it has to have its subnodes defined, and therefore it is element that is proper grammar rule and therefore it has to be somewhere in Initial Grammar, thus it is already processed or is on schedule. % TODO anti Totally unclear, rewrite
\end{itemize}
...
Call its submodule \jmodule{AutomatonSimplifier} to modify PTA to some generalized automaton by merging states.
...
\end{enumerate}
The data flow is in fig. \ref{automaton_merging_state_flow}.
...
To automaton, instead of instances on right side of rules, their cluster representants are inserted as one can see in section \ref{section_AutomatonMergingState_structure}. % TODO anti Don't understand the sentence
This assures that comparisons in automaton are done using Java \code{Object.equals()} reference comparison, but for each cluster member, its representant is only present in automaton.
That results in cluster-equal behaviour.
Example of prefix-tree automaton is in fig. \ref{pta}.
...
Module \jmodule{AutomatonSimplifier} implements solution searching logic in simplifying automaton by merging states. % TODO anti Don't understand
...
This cleaner does exactly what we have just described. % TODO anti Doesn't make sense
...
} until (user selected at least one state in last trial) TODO anti Trial -> Attempt?
\end{verbatim} 
...
We have implemented \emph{k,h-context} state equivalence (see \cite{ahonen}) in class \code{KHContext} (implements \code{Mer\.ge\.Con\.di\.tion\.Tes\.ter} interface), which is used by \code{Greedy} to test mergeability of states by default configuration. % TODO anti Sentence unclear
% TODO vektor : mention this? (we have stupid algorithm:) TODO anti Of course;)
...
% TODO anti Emphasise the states in following sentences
Before the algorithm starts, states superInitial and superFinal are added to automaton, former with $\lambda$-transition to initial state.
From all final states a $\lambda$ transition to superFinal state is added.
...
% TODO anti Emphasise the states again
...
\end{enumerate}
The whole process is step-by-step illustrated in fig. \ref{regaut}.
\begin{figure}[pb]
	\centering
