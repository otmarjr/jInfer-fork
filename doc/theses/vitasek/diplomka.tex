...
This chapter will be structured in the following way: first we will discuss the experimental data we used, then the methodology used in conducting the experiments, followed by the actual list of experiments with their full descriptions and results, and in the end we shall draw some final conclusions.
...
TODO describe the actual HW/SW setup: two computers, Windows/Linux, versions etc.

\subsection{Methodology}

What is running when experimenting, that we are always using more runs to counter for effects like caches and stuff that happens. Say many things actually depend on the order in which we run the experiments in one set, and if it is relevant we will talk about it in that particular experiment.

\subsection{Obtaining the results}

TODO we are logging (link to trace appendix) and creating CSVs.
