...
Following data sets have the same optimal ID sets regardless of the setting of $\alpha$ and $\beta$: \dataset{100-1000}, \dataset{MSH}, \dataset{NTH}, \dataset{XMA-c}, \dataset{XMA-p}.

TODO remaining datasets showed various dependencies on alpha and beta, because this question is not central to our interest we shall mention only 2 examples

TODO in 100-200 we have a clear boundary, in OVA1 (representative of OVAs in general) we have unclear boundary

\subsubsection{Results for \dataset{100-200}}
...
TODO play around with ratio in Mutation, but perhaps this is not enough - in that case move this after chained IHs, and try to tweak the best one by fiddling around with ratios.
...
TODO we will try 3 different scenarios, see which performs the best on the data and then try to tweak it a bit.
