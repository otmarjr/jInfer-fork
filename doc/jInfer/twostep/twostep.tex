...
While this is the only simplifier bundled with jInfer, its strong modularity ensures that it can be extended easily. For most of the points of extension we indeed provide multiple implementations. These vary from trivial proof-of-concept to for example $k,h$-context method for grammar rules inference (see \cite{ahonen}). Strong emphasis is put on generic automata, therefore it is easy to plug-in any automaton merging state algorithm.
...
To be able to respond to calls to \code{T getRepresentantForItem(T item)} method, every clusterer implementation has to keep all its clusters even after the clustering is finished. Using this method, given an item the correct cluster representative for this item has to be returned.
