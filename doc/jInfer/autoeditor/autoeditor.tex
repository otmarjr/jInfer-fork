...
\begin{verbatim}
public static <T> Layout<State<T>, Step<T>> createUserLayout(Automaton<T> automaton,
...
System of modules is described in documentation of Architecture, TODO ref na bib.
...
\begin{figure}
\centering\includegraphics[scale=\myscale]{uml_derived_classes}
...
And upon construction, it just adds \code{VertexPickingGraphMousePlugin} to the plugins of \code{PluggableVisualizer}. Purpose of this plugin is to allow user to pick some state of automaton and then call \code{guiDone()} method on instance of \code{AbstractComponent}. For description of \code{guiDone()} method, see \ref{subsubsection:AbstractComponent}.\\
...
Source codes of \code{StatePickingComponent} resides in package \code{cz.cuni.mff.ksi.jinfer.autoeditor.gui.component}, \code{StatePickingVisualizer} in \code{cz.cuni.mff.ksi.jinfer.autoeditor.automatonvisualizer} and \code{VertexPickingGraphMousePlugin} in \code{cz.cuni.mff.ksi.jinfer.autoeditor.automatonvisualizer.graphmouseplugins}.\\

Example of usage follows.
\begin{verbatim}
    final Transformer<Step<Regexp<T>>, String> transformer = new Transformer<Step<Regexp<T>>, String>() {
    ...
    final StatePickingVisualizer<Regexp<T>> visualizer = new StatePickingVisualizer<Regexp<T>>(LayoutHelperFactory.createUserLayout(automaton, transformer), transformer, component, automaton.getSuperInitialState(), automaton.getSuperFinalState());
...
Source codes of \code{StatesPickingComponent} resides in package \code{cz.cuni.mff.ksi.jinfer.autoeditor.gui.component}, \code{StatesPickingVisualizer} in \code{cz.cuni.mff.ksi.jinfer.autoeditor.automatonvisualizer} and \code{VerticesPickingGraphMousePlugin} in \code{cz.cuni.mff.ksi.jinfer.autoeditor.automatonvisualizer.graphmouseplugins}.\\

Example of usage follows.
\begin{verbatim}
  Automaton<T> simplify(Automaton<T> inputAutomaton, final SymbolToString<T> symbolToString, final String elementName) {
    if (!askUser) {
    ...
    do {
      final StatesPickingVisualizer<T> visualizer = new StatesPickingVisualizer<T>(LayoutHelperFactory.createUserLayout(inputAutomaton, transformer), transformer);
