...
\begin{enumerate}
\item \textbf{Construction of a syntax tree}. We use lexical and syntax analyses proposed in \cite{thesis_schejbal} and for each XQuery on input, we construct a data structure defined in Definition \ref{DEF_syntax_tree}.
\item \textbf{Static Analysis of expression types}. The algorithm searches for expressions in the syntax trees and statically (without evaluation) determines their types. See Section \ref{SECTION_expression_types}.
...
The first step of the algorithm involves lexical and syntax analyses known from the construction of compilers and produces a so-called syntax tree. The analyses are taken from Ji\v r\'{i} Schejbal's master thesis \cite{thesis_schejbal}. Since they are not directly related to the inference, and thus, they are not directly related to the topic of this thesis, we will not describe them. Nevertheless, they provide us with a helpful processing of XQuery queries and we can focus on the inference.
...
\item $c: V \rightarrow C$ is function assigning each node with its class from set $C$ of all XQuery language constructs listed in Tables \ref{TAB_syntax_tree_node_types_part_1}, \ref{TAB_syntax_tree_node_types_part_2},
...
\label{TAB_syntax_tree_node_types_part_1}
\end{table}
...
\label{TAB_syntax_tree_node_types_part_2}
\end{table}
...
An important characteristic of the syntax tree is related to a definition of local variables and their scope in the XQuery language. The representation of a definition of a local variable in the syntax tree is a node of \texttt{VariableBindingNode} class. Nodes of that class have only two children; a node representing the type of the variable and a node representing the binding expression (expression defining the value of the variable, and thus, it cannot use the variable). Hence, the entire subtree does not contain any expressions that use the variable. Therefore the scope of the new variable is not the subtree of the \texttt{VariableBindingNode} class node. It depends on the type of XQuery construct that the variable binding is an (indirect) component of.
...
A syntax tree constructed from sample query in Listing \ref{listing_test_query_18} is shown in Figure \ref{FIG_syntax_tree_1}.
...
\subsection{Expression Types} \label{SECTION_expression_types}
\begin{figure}
...
\begin{itemize}
\item XML Schema built-in atomic types. See Figure \ref{FIG_xsd_built_in_atomic_types}.
...
\subsection{PathType}
PathType contains additional information. The represented path is contained by a list of its steps, in particular instances of \texttt{StepExprNode}. If a step is a reference to a variable whose type is PathType, we also want to include this information. Therefore, PathType contains association between the steps and other PathTypes and this association is defined for the PathType variable steps.

To distinguish between a common PathType selecting a set of nodes and a PathType bound to a for variable in a FLWOR expression, PathType structure contains a flag \texttt{isForBound}.
...
\begin{itemize}
\item \texttt{steps} - A list of \texttt{PathExprNode} instances.
\item \texttt{substeps} - An association between variable-referencing steps and instances of PathType type.
...
\ELSIF{$is(exprNode,$ VarRefNode$)$}
     ...
     \IF{$type$ is PathType}
         ...
         \STATE $steps :=$ an array containing one item which is $step$
         \STATE $type :=$ PathType containing $steps$ and $substeps$
     \ENDIF
...
A parts of the function's semantic are separated in other functions, like the one in Algorithm \ref{ALG_func_determineOperatorType}, which is not presented completely because of the same reason.
...
\STATE $steps :=$ an empty array
\STATE $substeps :=$ an empty associative array
\FORALL{$step \in pathExprNode.getChildren()$}
	\STATE $detailNode := step.getChild(ExprNode)$
	\IF{$detailNode \neq$ null}
		\IF{$is(detailNode,$ VarRefNode$)$}
			\STATE $type := getType(detailNode)$ \COMMENT{PathType}
			\IF{$type.isForBound$}
				\STATE $substeps[stepNode] := type$
				\STATE add $step$ to $steps$
			\ELSE
				\STATE add all $type.steps$ to $steps$
			\ENDIF
		\ELSE
			\STATE add $step$ to $steps$
		\ENDIF
...
\RETURN $PathType$ with $steps$, $substeps$, and $isForBound$ set to false
\end{algorithmic}
\end{algorithm}
...
In this step, the algorithm discovers keys of elements, incorporating the approach from paper \cite{Necasky:2009:DXK:1529282.1529414}, described in Chapter \ref{Keys}, and extending it. Despite the approach was proposed, it has not been implemented yet. Therefore we will be the first to implement it and perform consecutive testing. 
...
Function \texttt{usesOnlyChildAndDescendantAxes} in Algorithm \ref{ALG_function_uses_only_child_and_descendant_axes} takes an instance of PathType as its arguments and returns true if the path uses only child, descendant and descendant or self axes. Otherwise, it returns false. Function \texttt{usesOnlyChildAnd\-DescendantAndAttributeAxes} is the same except it allows also attributes axis.
...
\FORALL{$step \in pathType.steps$}
	\IF{$step.isAxisStep$}
		\STATE $axisKind := step.axisNode.getAxisKind()$
		\IF{$axisKind \neq$ CHILD $\wedge$ $axisKind \neq$ DESCENDANT $\wedge$ $axisKind \neq$ DESCENDANT\_OR\_SELF}
			\RETURN false
		\ENDIF
	\ENDIF
\ENDFOR
...
\IF{$detailNode \neq$ null}
	\IF{$is(detailNode,$ VarRefNode$)$}
		\IF{$usesOnlyChildAndDescendantAxes(pathType.substeps[step]) =$ false}
			\RETURN false
		\ENDIF
	\ENDIF
\ENDIF
...
\FORALL{$step \in pathType.steps$}
	\IF{$step.hasPredicates$}
		\RETURN false
	\ENDIF
	\IF{$pathType.substeps[step] \neq$ null}
		\IF{$isWithoutPredicates(pathType.substeps[step] =$ false}
			\RETURN false
		\ENDIF
	\ENDIF
\ENDFOR
...
\STATE $lastStep :=$ the last item from $pathType.steps$
\IF{$lastStep.hasPredicates() =$ false}
	\RETURN false
\ENDIF
\IF{number of items in list $lastStep.getChild(PredicateListNode)$ $.getChildren()$ is higher than 1}
	\RETURN false
\ENDIF

\RETURN the only item in $lastStep.getChild(PredicateListNode)$ $.getChild(0)$
\end{algorithmic}
\end{algorithm}
...
\STATE $firstStep :=$ first item from  $pathType.steps$
...
A similar couple of functions is function \texttt{getTargetReturnPaths} and function \texttt{getTargetReturnPathsRecursive}. The only difference between those two couples of functions is that the former one searches for all expressions which are of PathType representing a target return path, while the later one searches only for instances of \texttt{PathExprNode}.
...
To define a key and a foreign key, we adopt the definitions introduced in the original approach \cite{Necasky:2009:DXK:1529282.1529414} with the following modifications.

\begin{itemize}
\item XPath paths are replaced by PathType instances.
\item All paths can use also descendant or self axis.
\item \emph{Key paths} can use also attribute axis.
\end{itemize}
...
A foreign key is a construct $(C, (P_1, \{L_1\}) \rightarrow (P_2, \{L_2\}))$, where $(C, P_2, \{L_2\})$ is a key and $P_1$ and $L_1$ are PathType instances without predicates and without special function calls that use only child, descendant, and descendant or self ($L$ also attribute) axes. $C$ can be omitted as in the case of keys.
\end{define}

An example of a global key is \texttt{(/site/people/person, \{@id\})}. An example of a foreign key to that key is  \texttt{((/site/closed\_auctions/closed\_auction, \{buyer/@person\}) $\rightarrow$ (/site/people/person, \{@id\}))}.
...
\IF{$P$ is of form $forVar/L_1 = curBindingNode.varName/L_2 \wedge usesOnlyChildAndDescendantAndAttributeAxes(L_1) \wedge usesOnlyChildAndDescendantAndAttributeAxes(L_2)$}
    ...
    \ENDIF
\ENDIF
\IF{$checkJoinPattern3 \wedge is(curBindingNode,$ ForClauseNode$) \wedge usesOnlyChildAndDescendantAndAttributeAxes(L_1) \wedge usesOnlyChildAndDescendantAndAttributeAxes(L_2)$}
    \IF{$whereExpr$ is of form $forVar/L_1 = bindingNode.varName/L_2$}
...
\begin{define}[Negative uniqueness Statement]
A negative uniqueness statement is a construct $(C, P)$, where $C$ and $P$ are PathType instances that use only child, descendant, and descendant or self ($P$ also attribute) axes. $C$ is called \emph{context path}, $P$ \emph{target path}. $C$ can be omitted as in the case of keys.
\end{define}
...
\IF{$is(node$, FunctionCallNode$)$}
    \IF{$node.fncName =$ 'distinct-values'}
        ...
        \IF{$type$ is PathType \\ $\wedge$ $usesOnlyChildAndDescendantAndAttributeAxes(type)$ \\ $\wedge$ $isWithoutPredicatesExceptLastStep(type)$}
...
Besides the \texttt{distinct-values} function, a certain form of FLWOR expressions is utilized in this step. They are FLWORs that iterate through a sequence of nodes selected by a path expressions using only child, descendant, and descendant or self axes, and either that path expressions end with a predicate comparing a value of a subnode with some other value (literal constant, reference to a variable which is constant in this expressions, etc), or, the comparison is located in a where clause of a respective FLWOR.

As discussed in Chapter \ref{Negative_uniqueness}, when a for clause is used to iterate through a set of items, it is expected that the number of the items may be more than one. And, together with that, the set is restricted to contain only items which satisfy a condition that a value of one of their subnodes (or descendant nodes) equals a certain value. That implies that the value in those nodes is not unique, and, alike the utilization of the aggregation functions, non-unique nodes cannot be keys. This situation is handled in Algorithms \ref{ALG_rejection_of_uniqueness_comparison_with_a_constant} and \ref{ALG_rejection_of_uniqueness_function_process_where}.
...
\IF{$is(exprNode,$ OperatorNode$)$}
    \IF{$exprNode.operator =$ GEN\_EQUALS}
        \FORALL{$var \in forVars$}
            \IF{$exprNode$ is of a form $var/L = C$ where $getType(var/L)$ is PathType $\wedge$ $isWithoutPredicatesExceptLastStep(getType(var/L))$ $\wedge$ $usesOnlyChildAndDescendantAndAttributeAxes(getType(var/L))$ $\wedge$ $C$ is a literal constant}
...
The original approach \cite{Necasky:2009:DXK:1529282.1529414} employs the following normalization which we adopt. It also incorporates a heuristic for precision enhancement. For details, we refer to it.

Let $K_1, \dots, K_n$ be the inferred keys. Let $S_i$ be the score (the sum of weights) of $K_i$  and $N_i$ be the number of the inferred statements about $K_i$.

Let $NU_1, \dots, NU_m$ be the discovered negative uniqueness statements. Let $W_j$ be the weight of $NU_j$.

The negative uniqueness statements affect the scores as follows. For each negative uniqueness statement $NU_j = (C^{NU}_j, P^{NU}_j)$, find $\mathcal{K}_j$ set of keys which are rejected by the statement. 
\begin{center}
$\mathcal{K}_j = \Bigl \{K_i = (C^K_i, P^K_i, \{L^K_i\}) \Big | C^{NU}_j = C^K_i, P^{NU}_j = P^K_i/L^K_i\Bigr \}$.
\end{center}
Then, for each key $K_i \in \mathcal{K}_j$ decrement its score $S_i$ with weight of the negative uniqueness statement $W_j$.

Let $S^{max}$ be the maximum from $|S_1|, \dots , |S_n|$ and $N^{max}$ be the maximum norm
from $N_1, \dots , N_n$. The normalized score $S_i$ of $K_i$ is computed as
\begin{center}
$S^{norm}_i = \frac{S_i}{S^{max}} * (1 - \frac{N^{max} - N_i}{\sum_{i=1}^{n}N_i})$
\end{center}
...
Figure \ref{FIG_syntax_tree_2} shows the syntax tree of the query in Listing \ref{listing_test_query_8} (subtree of \texttt{Construc\-torNode} is omitted because of lack of space). In this tree, the algorithm founds one occurrence of the join pattern 3. The two for clauses and one where clause which form the occurrence are shown in the red boxes.
