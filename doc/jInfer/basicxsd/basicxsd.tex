...
The main class implementing \code{SchemaGenerator} inference interface and simultaneously registered as its service provid\-er is \code{SchemaGeneratorImpl} in package \code{cz.cuni.mff.ksi.jinfer.basicxsd}. Process of export consists of two phases described in detail in later sections.
\begin{enumerate}
	\item Preprocessing.
	\item The export to a string representation itself.
\end{enumerate}
Method \code{start()} first creates an instance of \code{Preprocessor} class supplying the rules (elements) it got in the simplified grammar on input. Preprocessing itself is done by creating that instance (calling its constructor) and its results can be then retrieved by calling \code{getResult()} method. Purpose of preprocessing is to discover information necessary for export of elements. For example, determine elements which should be globally defined or which element is the root element.\\
...
As mentioned above, preprocessing is implemented in \code{Preprocessor} class and its is the following following.
\begin{itemize}
...
If it is desired to not generate global types, this feature can be turned off in preferences. However, turning it off is not recommended and in some cases it may cause invalidity of resulting XSD output. For more information, see \ref{section:problems}.\\

Minimal number of occurrences of an element to define its type globally can be altered if preferences as well. See \ref{subsection:preferences}.
...
Number of spaces characters per one level of indentation can be altered in project properties. See \ref{subsection:preferences}.

\subsubsection{Definition of elements}

Before we describe the export of elements, let's take a look on how we define elements and their attributes using XSD language. This subsection covers only XSD features that the exporter supports. For list of supported features, see \ref{subsection:supportedXSDFeatures}\\
...
There are two classes extending \code{AbstractElementsExporter}. \code{GlobalElementsExporter} and \code{RootElementExporter}. Their constructors has the same signature as constructor of \code{AbstractElementsExporter} and both have \code{run()} method to perform their function.\\
...
Export of attributes is done alongside with export of elements, as attributes are exported as XSD element \code{xs:attribute} in XSD element \code{xs:complexType}.
...
This module is just basic implementation, thus many XSD features are not supported. List of supported XSD elements and their attributes follows. Let \code{xs} be XMLSchema namespace.
...
\begin{enumerate}
	...
	\item Root element and recursively all remaining elements are exported to a XSD representation by \code{RootElementExporter}.
