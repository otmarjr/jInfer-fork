...
If the operator in an arithmetic operation is one of \texttt{+}, \texttt{-}, \texttt{*}, \texttt{div}, \texttt{mod} (the class of the expression node is \texttt{Operator} and it represents one of \texttt{PLUS}, \texttt{MINUS}, \texttt{MUL}, \texttt{DIV}, \texttt{MOD} (for all operators, see Attachment \ref{FIG_operators})), one operand is of PathType $P$ and the type $T$ of the other operand is one of numeric built-in types, then the inferred statement is $P \rightarrow T$.

\todo[inline]{Toto je zlozitejsie.}
