...
The two join patterns from the original approach are shown in Listings \ref{listing_for_join_pattern} and \ref{listing_let_join_pattern}. Additional join patterns are introduced in Listings \ref{listing_join_pattern_3}, \ref{listing_join_pattern_4} and \ref{listing_join_pattern_5}. In all join patterns $P_1$, $P_2$, $L_1$, $L_2$ are XQuery paths without predicates, using only child and descendant axes.
...
\begin{lstlisting}[mathescape, float, caption=Join pattern 4, frame=single, label=listing_join_pattern_4]
for $\$e_1$ in $P_1$
let $\$e_2$ :=
    $P_2[L_2 = \$e_1/L_1]$
return $C_R$
...
\begin{lstlisting}[mathescape, float, caption=Join pattern 5, frame=single, label=listing_join_pattern_5]
for $\$e_1$ in $P_1$
for $\$e_2$ in $P_2$
