...
\end{itemize}
As practice showed, for methods handling and infering structural properties, it is important to recognize whether structural node on input is element or text node. % TODO anti Unclear, rewrite
However methods for content devising don't need to know if they are working on infering model for content of attribute or text node. % TODO anti Unclear, rewrite
...
In result, \code{Element} and \code{SimpleData} have method \code{getType()} to devise type of \code{Abstr\.act\.Stru\.ctu\.ral\.Node} variables. % TODO anti What is "devise"?
And \code{SimpleData} and \code{Attribute} have methods \code{getContentType()} and \code{getContent()} to work with content model.
% TODO anti Last 2 sentences make no sense to me...
...
Take care of symbol class and its \code{equals()} implementation. % TODO anti What does it mean, to take care?
...
One can ask for merging states, that are merged out from automaton already. % TODO anti What does it mean?
