...
\FORALL{$bindingNode \in bindingNodes$}
    ...
    \IF{$getType(BindingExpr) == PathType$ using only child and descendant axes, ending with exactly one predicate $P$}
      \FORALL{$(var, node) \in forVars$}
            \IF{$P$ is of form $bindingNode.varName/L_1 = var/L_2$}
                \IF{$is(bindingExpr, ForClauseNode)$}
                    \STATE memorize $(node, bindingExpr)$ as an occurrence of the for join pattern
                \ELSIF{$is(bindingExpr, LetClauseNode)$}
                    \STATE memorize $(node, bindingExpr)$ as an occurrence of the for let pattern                
                \ENDIF
            \ENDIF
        \ENDFOR
