...
import cz.cuni.mff.ksi.jinfer.base.interfaces.Runner;
...
/**
 * RunnerImpl is responsible for running other modules in process of inference,
 * ensuring correct order and passing data from output of a module
 * to input of another.
 * 
 * @author rio
 */
@ServiceProvider(service = Runner.class)
public class RunnerImpl implements Runner {

  private static final Logger LOG = Logger.getLogger(RunnerImpl.class);
  private IGGenerator igGenerator;
  ...
  private final IGGeneratorCallback iggCallback = new IGGeneratorCallback() {
    ...
    public void finished(final List<AbstractNode> grammar) {
      RunnerImpl.this.finishedIGGenerator(grammar);
    }
  };
  private final SimplifierCallback simplCallback = new SimplifierCallback() {
    ...
    @Override
    public void finished(final List<AbstractNode> grammar) {
      RunnerImpl.this.finishedSimplifier(grammar);
    }
  };
  private final SchemaGeneratorCallback sgCallback = new SchemaGeneratorCallback() {
    ...
    public void finished(final String schema, final String extension) {
      RunnerImpl.this.finishedSchemaGenerator(schema, extension);
    }
  };
  ...
  public void run() {
    final Properties projectProperties = RunningProject.getActiveProjectProps(
            ModuleSelectionPropertiesPanel.NAME);
  ...
  public void finishedSchemaGenerator(final String schema, final String extension) {
    ...
    final String namePattern = NbPreferences.forModule(RunnerPanel.class).get(
            RunnerPanel.NAME_PATTERN, RunnerPanel.NAME_PATTERN_DEFAULT);
  ...
  private static void unexpected(final Throwable t) {
    ...
    final NotifyDescriptor message = new NotifyDescriptor.Message(
            "Process of inferrence caused an unexpected error:\n\n" + t.toString() + "\n\n Detailed information was logged to the logfile and inferrence was cancelled.",
  ...
  @Override
  public AbstractPropertiesPanel getNewProjectWizardPanel(final Properties properties) {
