...
\FORALL{$stepNode \in pathType.stepNodes$}
	\IF{$stepNode.hasPredicates()$}
		\RETURN false
	\ENDIF
	\IF{$pathType.forBoundSubsteps[stepNode]$ $!=$ $null$}
...
Function $endsWithExactlyOnePredicate$ checks if a path represented by a given $PathType$ instance contains exactly one predicate in its last step and if so, the predicate is returned. Otherwise, it returns false.
